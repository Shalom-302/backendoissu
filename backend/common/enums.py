...
class Discipline(StrEnum):
    """Sport disciplines covered by the V1 demonstration dataset."""
...
class AthleteCategory(StrEnum):
    """Age categories used by the OISSU school/university championships."""
...
class EducationLevel(StrEnum):
    """Education level of the athlete."""
...
class MedalType(StrEnum):
    """Distinction obtained on a performance.

    A result without a distinction stores ``NULL``, not a fourth "none" member:
    the medal tallies are plain ``COUNT(medal)`` that way.
    """
