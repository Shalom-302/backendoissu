...
class Settings(BaseSettings):
    ...
    FASTAPI_TITLE: str = 'OISSU CONNECT'
    FASTAPI_VERSION: str = '1.0.0'
    FASTAPI_DESCRIPTION: str = (
        'Suivi et traçabilité des performances des athlètes du sport scolaire '
        'et universitaire. V1 — deux rôles: USER (athlète) et ADMIN.'
    )
    ...
    TOKEN_REDIS_PREFIX: str = 'oissu:token'
    USER_SECURE_TOKEN_REDIS_PREFIX: str = 'oissu:user:token'
    ADMIN_SECURE_TOKEN_REDIS_PREFIX: str = 'oissu:admin:token'
    TOKEN_REFRESH_REDIS_PREFIX: str = 'oissu:refresh_token'
    CAPTCHA_LOGIN_REDIS_PREFIX: str = 'oissu:captcha:login'
    # JWT / RBAC whitelist. The API is mounted at the root (see backend/main.py),
    # so these are the real request paths.
    ...
    # JWT
    JWT_USER_REDIS_PREFIX: str = 'oissu:user'
    JWT_ADMIN_REDIS_PREFIX: str = 'oissu:admin'
    JWT_USER_REDIS_EXPIRE_SECONDS: int = 60 * 60 * 24 * 7

    # Permission (RBAC)
    PERMISSION_MODE: Literal['casbin', 'role-menu'] = 'casbin'
    PERMISSION_REDIS_PREFIX: str = 'oissu:permission'
    ...
    # Cookies
    COOKIE_REFRESH_TOKEN_KEY: str = 'oissu_refresh_token'
    ...
    LOG_STDOUT_FILENAME: str = 'oissu_access.log'
    LOG_STDERR_FILENAME: str = 'oissu_error.log'
    ...
    # CORS
    # Overridable from the environment as a JSON list, e.g.
    # CORS_ALLOWED_ORIGINS=["https://staging.oissu-connect.ci"]
    # Never use '*' here: the API answers with credentials (refresh cookie).
    ...
    # Request limiter
    REQUEST_LIMITER_REDIS_PREFIX: str = 'oissu:limiter'
    ...
    IP_LOCATION_REDIS_PREFIX: str = 'oissu:ip:location'
