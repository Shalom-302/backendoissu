...
from backend.app.admin.schema.user import UserLoginSchema
...
class AuthService:
    ...
    # NOTE — there is deliberately no `register()` here.
    # V1 exposes no public sign-up (doc §4.1, §11.1): the only way an athlete
    # account is created is `AthleteService.create`, which is ADMIN-only and
    # writes the account and the profile in one transaction.
        ...
        async with async_db_session.begin() as db:
            try:
                ...
                response.set_cookie(
                    ...
                    httponly=True,
                    # Browsers reject SameSite=None without Secure, so both come
                    # from the same settings as the refresh path below.
